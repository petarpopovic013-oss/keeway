  ...
  return (
    <div className="w-full max-w-4xl mx-auto my-20 px-4">
      <div className="flex flex-col items-center mb-10">
        <h2 className="text-2xl md:text-3xl font-saira font-bold not-italic text-black uppercase tracking-tight mb-6 text-center">
        ...
        <div className="flex gap-2 md:gap-4 w-full justify-center md:justify-end">
          <button 
            onClick={collapseAll}
            className="flex items-center gap-1 md:gap-2 border border-gray-300 rounded-full px-3 md:px-4 py-1.5 text-[10px] md:text-xs font-saira font-normal hover:bg-gray-50 transition-colors"
          >
        ...
        {validCategories.map((cat, idx) => {
          ...
          return (
            <div key={idx} className="border-b border-gray-200">
              ...
              >
                <span className="font-saira font-bold not-italic text-[17px] text-[#222]">{cat.title}</span>
                    ...
                    <div className="pb-8 pt-2 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-8">
                      {cat.items.map((item, itemIdx) => (
                        <div key={itemIdx} className="flex flex-col">
                          <span className="text-[15px] font-saira font-bold not-italic text-[#333] mb-1.5">
                            {item.label}
                          </span>
                          <span className="text-[15px] font-saira font-normal not-italic text-gray-500">
                            {item.value}
                          </span>
                        </div>
                      ))}
