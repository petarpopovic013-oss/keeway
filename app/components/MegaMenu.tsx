...
export default function MegaMenu({ isOpen, onClose, offsetLeft = 0 }: MegaMenuProps) {
  const [activeCategory, setActiveCategory] = useState<string>("Scooters");
    ...
    async function fetchMotorcycles() {
      ...
      if (data) {
        ...
        const categoryMotos = data.filter(
          (m: Motorcycle) => m.category.toLowerCase() === "scooters" || m.category.toLowerCase() === "scooter"
        );
          ...
          <div className="w-1/2 lg:w-auto lg:pr-8 border-r border-gray-200 pt-4 lg:pt-8 pb-4 lg:pb-8 lg:h-full overflow-y-auto">
            <ul className="flex flex-col gap-4 lg:gap-3">
              {CATEGORIES.map((cat) => (
                <li
                  key={cat}
                  className={`cursor-pointer font-saira font-bold not-italic text-[13px] lg:text-base tracking-tight uppercase transition-colors ${
            ...
            <ul className="flex flex-col gap-4 lg:gap-3">
              {currentCategoryMotos.length > 0 ? (
                currentCategoryMotos.map((moto) => (
                  <li
                    key={moto.id}
                    className={`cursor-pointer font-saira font-normal not-italic text-[13px] lg:text-base tracking-wide transition-colors ${
                  ...
                  >
                    <>
                      ...
                      <Link href={`/motocikli/${moto.slug}`} onClick={onClose} className="hidden lg:block w-full text-left">
                        {moto.name.replace('Keeway ', '').replace('MBP ', '')}
                      </Link>
                ...
                <li className="text-gray-400 text-sm font-zuume font-normal italic">Nema modela</li>
              )}
            </ul>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center justify-start w-full lg:h-full">
            ...
            {activeMotorcycle && (
              <Link 
                ...
                onClick={onClose}
                className="lg:hidden w-full bg-track-cyan hover:bg-black transition-all duration-300 text-white font-zuume font-normal italic text-center py-4 uppercase tracking-widest text-[13px] mt-6 shadow-[0_4px_14px_0_rgba(66,210,242,0.39)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.23)]"
              >
                Idi na model
              </Link>
            )}
