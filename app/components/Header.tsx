...
export default function Header() {
  ...
  return (
    ...
    >
      <div className="w-full px-4 xl:px-12 mx-auto max-w-[1440px] flex items-center h-full justify-between">
        ...
        <div className="flex-1 flex items-center justify-center lg:flex-none lg:justify-start">
          <Link href="/" className="flex items-center relative z-10 shrink-0">
            <Image
              src="/logos/Keeway-Worldmark-Orange.png"
              alt="Keeway Logo"
              width={220}
              height={50}
              className={`transition-all duration-300 w-[150px] lg:w-[220px]`}
              priority
            />
          </Link>
        </div>
        ...
        <div className="lg:hidden flex items-center justify-end shrink-0 w-1/4">
          ...
          >
            <span className={`font-zuume font-normal italic text-sm ${isSolid ? "text-black" : "text-white"}`}>MODELI</span>
        ...
        <nav
          className="hidden lg:flex flex-1 items-center justify-start lg:ml-12 xl:ml-20 gap-4 xl:gap-8 font-zuume font-normal text-base xl:text-xl tracking-[0.03em] h-full uppercase italic"
        ...
        >
          ...
          <div className="flex items-center h-full">
            ...
            >
              <span className={`font-zuume font-normal italic ${isSolid ? "text-black" : "text-white"}`}>MODELI</span>
              <ChevronDown className={`w-4 h-4 ${isSolid ? "text-black" : "text-white"} group-hover:text-track-cyan`} />
            </div>
          </div>

          {/* Orange Divider */}
          <div className="w-[2.5px] h-[25px] bg-keeway-orange rounded-full mx-[-8px]"></div>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 pb-2">
          <span className="font-zuume font-normal italic text-lg text-black">Meni</span>
          <button
            onClick={() => setIsMobileMenuOpen(false)}
        ...
        <div className="flex flex-col mt-4">
          <Link
            href="/"
            className={`flex items-center justify-between p-6 py-5 border-b ${pathname === '/' ? 'text-[#F54308] border-[#F54308]' : 'text-black border-gray-100'}`}
            onClick={() => setIsMobileMenuOpen(false)}
          >
            <span className="font-zuume font-normal italic text-base">Naslovna</span>
          ...
          <Link
            href="/novosti"
            className={`flex items-center justify-between p-6 py-5 border-b ${pathname === '/novosti' ? 'text-[#F54308] border-[#F54308]' : 'text-black border-gray-100'}`}
            onClick={() => setIsMobileMenuOpen(false)}
          >
            <span className="font-zuume font-normal italic text-base">Novosti</span>
            <svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M1 1L6 6L1 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </Link>
