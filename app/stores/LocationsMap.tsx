...
function createMarkerIcon(group: LocationGroup, isActive: boolean) {
  ...
  return L.divIcon({
    className: "keeway-map-marker-wrapper",
    html: `<span class="keeway-map-marker ${typeClass}${isActive ? " is-active" : ""}"><span></span></span>`,
    iconSize: [34, 42],
    iconAnchor: [17, 40],
    popupAnchor: [0, -38],
  });
...
function LocationMarker({
  ...
  return (
    <Marker
      ...
      title={title}
      alt={title}
      eventHandlers={{ click: () => onSelect(group.locations[0].id) }}
    >
      <Popup className="keeway-map-popup" minWidth={245} maxWidth={290}>
        <div className="font-saira not-italic text-black">
            ...
            >
              <span
                className={`mb-1 block text-[10px] font-bold uppercase tracking-[0.14em] ${
                  location.type === "sales" ? "text-[#F54308]" : "text-[#003A73]"
                }`}
              >
              ...
              <div className="mt-3 flex flex-col gap-1 text-xs">
                <a className="font-medium hover:text-[#F54308]" href={`tel:${location.phoneHref}`}>
                  {location.phone}
                </a>
                <a className="break-all hover:text-[#F54308]" href={`mailto:${location.email}`}>
                  {location.email}
                </a>
                <a
                  className="mt-2 font-bold uppercase tracking-wide hover:text-[#F54308]"
...
export default function LocationsMap({ locations, activeId, onSelect }: LocationsMapProps) {
  ...
  return (
    <MapContainer
      ...
      aria-label="Mapa ovlašćenih Keeway prodajnih mesta i servisa"
    >
      <TileLayer
