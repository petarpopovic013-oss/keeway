...
Ovaj dokument služi kao "context engineering" fajl koji pruža tačan pregled trenutne arhitekture baze podataka za projekat "Morbidelli + Keeway". Baza je smeštena na Supabase platformi i dizajnirana je tako da podrži oba sajta u istoj bazi, ali sa potpuno odvojenim tabelama i Storage bucket-ima.

> [!NOTE]
> Za ovaj konkretan sajt (Keeway) koristiće se **isključivo** tabele i bucketi sa prefiksom `keeway_`. Morbidelli tabele i bucketi postoje u istoj bazi, ali se koriste samo za Morbidelli aplikaciju.
...
Sadrži sve slike vezane za Keeway ponudu. Nema Row Level Security (RLS) restrikcija za čitanje, što znači da bilo ko može pristupiti slikama putem javnog URL-a. Ovo se koristi na Keeway aplikaciji za prikaz slika.

### `morbidelli_images` (Public)
Sadrži sve slike vezane za Morbidelli ponudu. Nema RLS restrikcija. Koristi se isključivo za Morbidelli sajt.
...
Ovo je glavna i jedina tabela koju koristi Keeway sajt za katalog i prikaz motora.

| Kolona | Tip Podataka | Opis / Značenje |
| :--- | :--- | :--- |
| `id` | `uuid` | Primarni ključ, automatski se kreira (`gen_random_uuid()`). |
| `name` | `text` | Puni naziv motora (npr. "Keeway T1002VX"). |
...
| `image_url` | `text` | Putanja/URL do glavne slike unutar `keeway_images` bucketa. |
...
Ista strukturna šema kao gore, rezervisana za upotrebu na odvojenom sajtu za Morbidelli brend.
...
| `image_url` | `text` | Glavna slika u `morbidelli_images` bucket-u. |
| `gallery` | `text[]` | Dodatne slike (galerija). |
| `specifications` | `jsonb` | Fleksibilne specifikacije. |
| `...` | | Ista specifikacijska polja (50+ komada) kao kod `keeway_motorcycles`. |
