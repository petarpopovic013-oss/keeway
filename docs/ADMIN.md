# Keeway Admin Panel - Tehnička Dokumentacija

Ovaj dokument opisuje tehničku arhitekturu i način rada Admin panela (`/admin`) unutar Keeway web aplikacije.
...
- **Javni deo sajta (Frontend)**: Tabele (`keeway_motorcycles`) i Storage bucketi (`keeway_images`) imaju otvorene `SELECT` (čitaj) dozvole, tako da aplikacija može javno dohvatiti podatke koristeći `SUPABASE_ANON_KEY`.
...
- **Auto-generisanje slug-a**: Funkcija koja pri unosu naziva motora u realnom vremenu pravi URL-friendly `slug` (npr. "Keeway V21" -> "keeway-v21"). Ukoliko se motor menja, slug se ne generiše automatski da se ne bi prekinuli postojeći linkovi, ali admin ga i dalje može ručno prepraviti.
